...
/// Access to the `address` unique index on the table `player`,
/// which allows point queries on the field of the same name
/// via the [`PlayerAddressUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.player().address().find(...)`.
pub struct PlayerAddressUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<Player, String>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> PlayerTableHandle<'ctx> {
    /// Get a handle on the `address` unique index on the table `player`.
    pub fn address(&self) -> PlayerAddressUnique<'ctx> {
        PlayerAddressUnique {
            imp: self.imp.get_unique_constraint::<String>("address"),
            phantom: std::marker::PhantomData,
        }
    }
}

impl<'ctx> PlayerAddressUnique<'ctx> {
...
pub(super) fn register_table(client_cache: &mut __sdk::ClientCache<super::RemoteModule>) {
    ...
    _table.add_unique_constraint::<String>("address", |row| &row.address);
}
