...
/// Gera um veículo (bike simples)
pub fn create_bike_mesh() -> Mesh {
    let mut mesh = Mesh::_new(bevy::render::render_resource::PrimitiveTopology::TriangleList);

    // Quadro (reta)
    let frame_positions = vec![
        [-1.0, 0.0, 0.0], // Roda traseira
        [1.0, 0.0, 0.0],  // Roda dianteira
        [0.0, 0.5, 0.5],  // Assento
    ];

    let frame_indices = vec![[0, 1, 2]];

    // Rodas (círculos)
    let wheel_radius = 0.4;
    let wheel_segments = 12;

    let mut wheel_positions = Vec::new();
    for i in 0..wheel_segments {
        let angle = std::f32::consts::TAU * i as f32 / wheel_segments as f32;
        wheel_positions.push([wheel_radius * angle.cos(), wheel_radius * angle.sin(), 0.0]);
    }

    let mut wheel_indices = Vec::new();
    for i in 0..wheel_segments {
        let next = (i + 1) % wheel_segments;
        wheel_indices.push([i as u32, next as u32, (i + 1) as u32]);
    }

    // Mover rodas para posições corretas
    wheel_positions[0] = [-1.0, 0.0, 0.0]; // Traseira
    wheel_positions[1] = [1.0, 0.0, 0.0]; // Dianteira

    let mut all_positions = frame_positions;
    all_positions.extend(wheel_positions);

    let mut all_indices = frame_indices;
    all_indices.extend(
        wheel_indices
            .iter()
            .map(|&i| i + frame_positions.len() as u32),
    );
...
/// Sistema que spawn automaticamente os meshes no mundo
pub fn spawn_procedural_assets(mut commands: Commands) {
    // Spawn hexágonos do terreno
    for i in -10..=10 {
        for j in -10..=10 {
            let x = i as f32 * 10.0;
            let y = j as f32 * 10.0;
            ...
            commands.spawn((
                ...
                create_hex_mesh(5.0, 0.5),
                Name::new(format!("hex_{}_{}", i, j)),
            ));
        }
    }
